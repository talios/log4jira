...
public class JiraAppender extends AppenderSkeleton {
  ...
  protected void append(LoggingEvent event) {
    ...
    if (event.getLevel().toInt() >= Level.ERROR.toInt() && event.getThrowableInformation() != null) {
      ...
      Preferences node = Preferences.userNodeForPackage(JiraAppender.class);


      String storedDigest = node.get(digest, "");

      if (storedDigest == null || "".equals(storedDigest)) {
        ...
        try {
          ...
          node.put(digest, newIssueKey + ":" + new Date().getTime());
        ...
        try {
          ...
          String[] issueElements = storedDigest.split(":");
          String issueKey = issueElements[0];
          long issueTimestamp = new Long(issueElements[1]).longValue();

          // Only comment if older than 10 minutes - should be configurable
          long nowStamp = new Date().getTime();

          long checkStamp = nowStamp - delayPeriod;

          if (checkStamp > issueTimestamp) {


            JiraClientContainer clientContainer = getClientContainer();

            System.out.println("Adding comment to " + issueKey);
            List params = new ArrayList();
            params.add(clientContainer.token);
            params.add(issueKey);
            ...
            // Update timestamp
            node.put(digest, issueKey + ":" + new Date().getTime());
